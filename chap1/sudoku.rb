class Puzzle
  ...
  ASCII = ".123456789"
  BIN = "\000\001\002\003\004\005\006\007\010\011" # 0 is a placeholder, 1-9 are the actual values
  ...
  def has_duplicates?
    ...
    0.upto(8) do |row| return true if rowdigits(row).uniq! end
    0.upto(8) do |col| return true if coldigits(col).uniq! end
    0.upto(8) do |box| return true if boxdigits(box).uniq! end
  ...
  def coldigits(col)
    ...
    col.step(80, 9) { |i| # Loop from col to the last column up to 80
    ...
    puzzle.each_unknown do |row, col, box| # For each unknown cell
      ...
      when 1 # If there is only one possible value
        ...
        if unchanged && p.size < min # Is this the best cell so far?
          ...
          rmin, cmin, pmin = row, col, p # Remember the cell and values
        end
      end
    end
  end

  return rmin, cmin, pmin # Return the best cell and its possible values
end
